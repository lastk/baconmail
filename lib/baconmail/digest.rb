    ...
    def self.daily_digest(account, configs)
      ...
      Gmail.new(account.username, account.password) do |gmail|
        gmail.mailbox('[Gmail]/All Mail').emails(:on => (Date.today - 1)).each do |email|
          ...
          next if "#{email.from[0]['mailbox']}@#{email.from[0]['host']}" == username
        ...
        if emails.size > 0
              if configs.use_preview
                ...
                self.generate_previews(emails, configs, username)
              end
              ...
              digest.body            = self.email_template(emails, aib_domain, configs, username)
              digest.deliver!
        end
      end
